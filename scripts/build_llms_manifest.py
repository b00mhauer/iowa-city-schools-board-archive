...
def render_manifest(year: int, summaries: dict, all_meetings: list[dict],
    ...
    out.append(f"- **[General Fund cash seasonality & liquidity risk]({RAW_BASE}/cash-seasonality.md)** — independent community analysis of the intra-year cash cycle. Establishes a baseline monthly pattern from the district's own cash-flow schedules: property-tax peaks in October and April, the late-summer (Aug–Sep) trough that forces an interfund loan every year, and the FY27 shift of state aid to quarterly payments that migrates the trough toward March. Includes a seasonal-index table, days-cash-on-hand by month, and a forward-drawdown stress test for judging whether a given balance (e.g. ~30 days in June) is low, adequate, or comfortable for that time of year. Not produced by ICCSD or PFM.")
    out.append("")
